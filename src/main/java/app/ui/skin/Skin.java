...
/**
 * Ja, das ist hier heftige Reflection, es tut mir leid. Aber was ich wollte ist:
 * - Skins, die voneinander erben können
 * - Jedes Skin hat seine eigene Konfigurationsdatei
 * - Wenn in einem "niedrigeren" Skin eine Property definiert ist, die weiter oben im Baum auch nochmal definiert ist, gewinnt die untere
 * - Ein Ändern einer Property-Datei wird beim nächsten Neustart automatisch aktiv
 * - Die Werte können in den Skinklassen mittels einfacher Variablen genutzt werden. Kein get("textColor") nötig.
 */
public abstract class Skin {
	
    public enum IconButtonType {BACK, SKIP, PLAY, CANCEL};
    public enum TextLabelType {
        ...
        TextLabelType(final String text) {this.text = text;}
	...
	protected Color activeButtonBgColor; // MCLabels !Später Das ergibt wenig Sinn. Ein Button ist auch ein InputElement und sollte somit activeComponentBackgroundColor nutzen! 
	...
	/**
	 * Ist die Breite der ContentPane. Menüzeile und Border um die Rootpane sowie der Windows-Border gehören nicht dazu!
	 * --- Fenster = Spielfeld [+ Menü] + 2xBorder ---
	 * Von daher: Wenn deine Font größer wird bei einem Skinwechsel, dann vergrößert sich das Fenster
	 * @return
	 */
	...
	/**
	 * Das MainWindow muss sich bei Skinwechsel selbst redekorieren...
	 * @param mainWindow
	 */
	public void redecorateMainWindow(JFrame mainWindow) {
		JRootPane rootPane = mainWindow.getRootPane();
		// Hintergrund der Titelleiste rechts neben den Menüpunkten
		rootPane.putClientProperty("JRootPane.titleBarBackground", menuBarBackground);
		rootPane.putClientProperty("JRootPane.titleBarForeground", textColor);
		rootPane.setBorder(BorderFactory.createLineBorder(thinBorderColor, 1));
	}
    ...
    public JMenu createJMenu(String string) {
    	return new JMenu(string);
    }
    
    public JMenuItem createJMenuItem(String string) {
    ...
    public CustomTextField createInputField(DeckType type) {
    	...
    	CustomTextField result = new CustomTextField(font, textC, incorrectText, activeComponentBgColor, disabledComponentBgColor
    			, SwingConstants.CENTER, borderSmallComponent);
    ...
    public CustomButtonLabel createAnswerButton() {
    	...
    	return new CustomButtonLabel(font, smallFont, textC, activeBg, hoverBg, inactiveBg,
    			correctColor, incorrectColor, disabledBg, borderSmallComponent, null);
    }
	...
	public CustomButtonLabel createIconButton(DeckType type, IconButtonType buttonType) {
		...
		case SKIP:
			return new CustomButtonLabel(null, null, textColor, activeComponentBgColor, activeComponentHoverColor, null, null, null,
					disabledComponentBgColor, borderBackButton, skipButtonIcon);
		case PLAY:
			return new CustomButtonLabel(null, null, textColor, activeComponentBgColor, activeComponentHoverColor, null, null, null,
					disabledComponentBgColor, borderBackButton, playButtonIcon);
		case CANCEL:
			return new CustomButtonLabel(null, null, textColor, activeComponentBgColor, activeComponentHoverColor, null, null, null,
					disabledComponentBgColor, borderBackButton, cancelButtonIcon);
		default:
	...
	/**
	 * Erstellt ein Deutschlandpanel. Aktuell noch ohne Hintergrundbild. Die Größe berechnet sich aus:
	 * Höhe = height + 2 * yPadding. Der scale findet ohne Verzerrung statt, von daher ist die width
	 * dann auch aus diesen Parametern und der Auflösung der Map bestimmt.
	 */
	public ShapeMapPanel createShapeMapPanel(DeckType type) {
	    ...
	    ShapeMapPanel result = new ShapeMapPanel(map, bounds.height, activeComponentBgColor, borderColor, activeComponentHoverColor, correctColor, incorrectColor, disabledComponentBgColor, markedColor, 1.8f, 3.5f, 10, false, shapeMapColor0, shapeMapColor1);
	    result.setLocation(bounds.x, bounds.y);
	    return result;
	}
	...
	public String getMapImagePath(DeckType type) {
		switch (type) {
			case WORLD_CARDS: return Config.get("mapImagesFolder") + worldMapImageName;
			default: return null;
		}
	}
	...
	public String getMapInactiveImagePath(DeckType type) {
		switch (type) {
			case WORLD_CARDS: return Config.get("mapImagesFolder") + worldMapInactiveImageName;
			default: return null;
		}
	}
	...
	public String getMapInactiveOverlayImagePath(DeckType type) { 
		switch (type) {
			case WORLD_CARDS: return Config.get("mapImagesFolder") + worldMapInactiveOverlayImageName;
			default: return null;
		}
	}
     ...
     * 
     * @param c
     * @param intensity: an int value between 0 (no change) and 100 (maximal change will lead to white or black...)
     * @return
     */
    ...
    protected void configureUiManager() {
        ...
        UIManager.put("MenuBar.border", BorderFactory.createMatteBorder(0, 0, 1, 0, thinBorderColor)); // Farbe der Linie unter der Titel / Menüzeile des JFrames.
    	...
    	//!Architektur: Wir wollten setzen auf 1) Swings set-Methoden, 2) FlatLafStyles und 3) UIManager.puts wo es nicht anders geht. Das refactoring fehlt noch!
    	...
    	UIManager.put("Button.default.background", activeComponentBgColor); // Hintergrundfarbe des Default-Buttons (OK) wenn er nicht fokussiert ist, also nach Click auf einen anderen Button und dann Wegziehen der Maus...
        ...
        /**MenuBar direkt setzen für die Exceptions
        JMenuBar menuBar = mainWindow.getJMenuBar();
        menuBar.setForeground(newSkin.getTextColor());
	...
	protected BorderParams parseBorderParams (String value) {
		String[] values = value.split(",");
		if (values.length==7)
			return BorderParams.of(Integer.parseInt(values[0]),
					parseColor(values[1]), 
     ...
     * @param fgColor
     */
    /**public default void addDisabledColors(JComponent component, Color bgColor, Color fgColor) {
        Map<String, String> updates = Map.of(
            "disabledBackground", toHex(bgColor),
            "disabledText", toHex(fgColor)
        );
        updateFlatLafStyle(component, updates);
    }**/
}
