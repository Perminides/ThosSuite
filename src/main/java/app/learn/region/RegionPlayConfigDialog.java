...
public class RegionPlayConfigDialog {
    ...
    private final ComboBox<Mode> modeComboBox = new ComboBox<>();
    ...
    public RegionPlayConfigDialog(Window parent, Skin skin) {
        ...
        modeComboBox.setValue(Mode.ELIMINATION_REGION);
    ...
    private void updateCheckBoxStates() {
        Mode selectedMode = modeComboBox.getValue();
        ...
        for (Map.Entry<Deck, CheckBox> entry : deckCheckBoxes.entrySet()) {
            ...
            if ((selectedMode.getCapitalOrRegion() == Mode.CapitalOrRegion.CAPITAL ||
            		selectedMode.getCapitalOrRegion() == Mode.CapitalOrRegion.BOTH) && !type.hasCapital()) {
                disable = true;
            }
    ...
    private void updateModeComboBoxItems() {
    	...
    	List<Mode> kompatibel = Arrays.stream(Mode.values())
    	    .filter(mode -> {
    	        if (mode.getCapitalOrRegion() == Mode.CapitalOrRegion.CAPITAL ||
    	            mode.getCapitalOrRegion() == Mode.CapitalOrRegion.BOTH) {
        ...
        Mode current = modeComboBox.getValue();
