...
import app.shared.model.Selection;
...
public class ImageBatchProcessor {
            ...
            for (Path imgPath : stream.toList()) {
                ...
                if (Files.exists(target)) {
                    SkinService.get().showAlert("Achtung",
                            imgPath.getFileName() + " ist bereits verkleinert. Ich breche ab.", DialogButton.OK);
                    return;
                }
                ...
                Optional<Selection> choice = chooseImage(left, right);
                if (choice.isEmpty()) return;

                BufferedImage chosen = choice.get() == Selection.ZERO ? left : right;
    ...
    private static Optional<Selection> chooseImage(BufferedImage left, BufferedImage right) {
        Window owner = SkinService.getOwnerWindow();

        @SuppressWarnings("unchecked")
        Dialog<Selection> dialog = (Dialog<Selection>) SkinService.get().createDialog(owner, "Bild auswählen");
        ...
        okLeft.setOnAction(_ -> dialog.setResult(Selection.ZERO));
        okRight.setOnAction(_ -> dialog.setResult(Selection.ONE));
