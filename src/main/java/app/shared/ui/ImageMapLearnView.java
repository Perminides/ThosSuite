...
public class ImageMapLearnView extends AnkiLearnView {
	...
	public ImageMapLearnView(String deckId, String mapName, String category,
			Function<Set<String>, List<ShapeGeometry>> geometryFor, AnkiCallbacks callbacks) {
	...
	protected LearnMap createMap() {
		...
		ImageMapPane map = new ImageMapPane(
				skin.mapImages(mapName()),
				skin.imageMapOverlayContentInset(mapName()),
				skin.learnComponentBounds(deckId(), mapName(), category(), LearnComponent.MAP),
				geometryFor);
		map.setListener(callbacks().mapElementClicked());
		return map;
	}
